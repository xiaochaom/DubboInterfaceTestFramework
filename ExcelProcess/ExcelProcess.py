...
class ExcelProcess(object):
    ...
    def __get_by_xlrd(self, filePath, case_line_list, by_index=0):
        ...
        if json.dumps(table.row_values(0), encoding='UTF-8', ensure_ascii=False) == json.dumps(
                ["用例编号", "名称", "描述", "系统", "服务", "接口", "数据初始化", "数据恢复", "参数", "预期结果", "变量配置"], encoding='UTF-8',
                ensure_ascii=False):
            ...
            for row_num in case_line_list:
                ...
                tmpCase = {'case_id': row_values[0].encode('utf8'), 'name': row_values[1].encode('utf8'),
                           ...
                           'data_init': row_values[6].encode('utf8'),
                           'data_recover': row_values[7].encode('utf8'),
                           'params': type(row_values[8]) == float and str(int(row_values[8])) or str(row_values[8]).encode('utf8').replace("\n", "").strip(),
                           'expect': type(row_values[9]) == float and str(int(row_values[9])) or str(row_values[9]).encode('utf8'),
                           'varsconf': str(row_values[10]).encode('utf8').replace("\n", "").strip()}
    ...
    def __get_by_value(self, filePath, startLine, endLine, by_index=0):
        ...
        def CaseList(Case):
            ...
            tmpCase = {'case_id': row_values[0].encode('utf8'), 'name': row_values[1].encode('utf8'),
                       ...
                       'params': type(row_values[8]) == float and str(int(row_values[8])) or str(row_values[8]).encode('utf8').replace("\n", "").strip(),
                       'expect': type(row_values[9]) == float and str(int(row_values[9])) or str(row_values[9]).encode('utf8').replace("\n", "").strip(),
                       'varsconf': str(row_values[10]).encode('utf8').replace("\n", "").strip()}
            return tmpCase
...
if __name__ == "__main__":
    ...
    try:
        tables = excelProcess.get_case_list('E:\DubboInterfaceTestFramework\Run\Testcases\Testcases_sku.xlsx',
                                            [],2,100)
        for row in tables:
